  ...
  return fetch(`http://127.0.0.1:8000/get_user/${username}`, {
    ...
    .then(data => {
      if (data.statuscode !== 200) {
        ...
        submeterNames = Array.from(
          { length: submeterCount },
          (_, i) => `Submeter ${i + 1}`
        )
...
function associate (submeterSelect, submeterIndex) {
  ...
  for (let i = 1; i <= backendData.submeters; i++) {
    const otherSubmeterSelect = document.querySelector(
      `select[data-submeter="${i}"]`
    )
    if (otherSubmeterSelect) {
      const index = Array.from(otherSubmeterSelect.options).findIndex(
...
function enableNextDropdown (currentSubmeter) {
  ...
  const nextSubmeterSelect = document.querySelector(
    `select[data-submeter="${nextSubmeter}"]`
  )
...
function getRemainingItems () {
  const selectedItems = Object.values(associations).flatMap(
    association => association.items
  )
...
function submitData () {
  ...
  const username = localStorage.getItem('username')
  associations.username = username
  const associationsString = JSON.stringify(associations);
  function set_data(associationsString){
    
      fetch('http://127.0.0.1:8000/create_submeter/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: associationsString
      })
        .then(response => {
        ...
        .then(data => {
          console.log('Submeter created:', data);
          // Handle the response data as needed
        })
        .catch(error => {
          console.error('Error creating submeter:', error);
          // Handle errors
        });

    
  }
  set_data(associationsString)
  const homePath = '/login.html'
